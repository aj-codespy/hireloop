...
async def main():
    ...
    job_id = jobs[0]["id"]
    cands = await store._request("GET", "candidates", params={"select": "id", "limit": "1"})
    cand_id = cands[0]["id"]
    ...
    print(f"Created application {app_id} with token {token}")
        ...
        while True:
            try:
                ...
                if msg["type"] == "question":
                    idx = msg["index"]
                    print(f"Got question {idx}: {msg['text']}")
                ...
                elif msg["type"] == "scoring_complete":
